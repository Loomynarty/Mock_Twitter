...
<body>
    ...
    <?php
        ...
        $follow = $conn->prepare("select username from users where id in (select user_id from followers join users on (users.id = user_id) where follow_id = (select id from users where username = ?))");
